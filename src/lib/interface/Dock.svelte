<script lang="ts">
    ...
    console.log(dock.source, typeof dock.source)
    const values: any[] = [($ports.find(port => dock.id == dock.source.id)||{name: "Port"}).name, dock.conditions]
...
<div class="w-full h-auto bg-neutral-900 shadow-md shadow-neutral-950 rounded-md">
    ...
    <Button onClick={executeTrade}><h1 class="text-neutral-300">Trade</h1></Button>
    <Button onClick={upgradeDock}><h1 class="text-neutral-300">Upgrade</h1></Button>
    <div class="text-lg">
